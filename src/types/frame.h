...
CIG_INLINED  DECLNAME##_t  DECLNAME##_zero() { \
 ...
 \
CIG_INLINED  bool  DECLNAME##_contains(DECLNAME##_t F, VEC2 P) { \
  return P.x >= F.x && P.y >= F.y && P.x < F.x + F.w && P.y < F.y + F.h; \
} \
 \
CIG_INLINED  DECLNAME##_t  DECLNAME##_inset(DECLNAME##_t f, INSETS i) { \
  return (DECLNAME##_t) { \
    f.x + i.left, \
    f.y + i.top, \
    f.w - (i.left + i.right), \
    f.h - (i.top + i.bottom) \
                                                                                               ...
                                                                                               \
CIG_INLINED  DECLNAME##_t  DECLNAME##_offset_vec2(DECLNAME##_t frame, VEC2 offset) {           \
...
CIG_INLINED  bool  DECLNAME##_cmp(DECLNAME##_t lh, DECLNAME##_t rh) { \
 ...
 \
CIG_INLINED  VEC2  DECLNAME##_center(DECLNAME##_t f) { \
