...
void test_utf8_char_iter_previous() {
  utf8_string str = make_utf8_string("Hдこ😁");
  // Iterator at the end of the string
  utf8_char_iter iter = make_utf8_char_iter_at(str, str.str + str.byte_len);

  utf8_char ch;

  ch = previous_utf8_char(&iter);
  assert(ch.byte_len == 4);
  assert(strncmp(ch.str, "😁", ch.byte_len) == 0);
...
int main() {
  ...
  TEST(test_utf8_char_iter_next);
  TEST(test_utf8_char_iter_previous);
