...
static void do_taskbar() {
  ...
  CIG({
    ...
    /* Left: Start button */
    start_button(CIG_FILL_W(start_button_width));
    ...
    CIG_HSTACK({
      ...
      for (i = 0; i < this->running_apps; ++i) {
        for (j = 0; j < 1; ++j) {
          ...
          taskbar_button(CIG_FILL, wnd->title, wnd->icon, wnd == this->selected_window);
        }
      }
...
window_message_t begin_window(window_t *wnd) {
  ...
  CIG_HSTACK({
    ...
    if (standard_button(cig_rect_make(CIG_W_INSET - 16, 0, 16, CIG_FILL_CONSTANT), "X")) {
      msg = WINDOW_CLOSE;
    }
  }
  ...
  cig_push_layout_function(
    ...
    cig_rect_make(0, 18, CIG_FILL_CONSTANT, CIG_H_INSET - 18),
    cig_insets_zero(),
